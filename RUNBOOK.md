...
- **画面が更新されない / Socket.IO が不安定**
  ...
  - ステータスチップが「再接続中…」のまま戻らない場合：Pi5 サーバーの `toolmgmt.service`（または Docker `app`）が起動しているかを確認し、必要に応じて Pi5 側を再起動する。
  - ステータスチップが `OFFLINE` に降格する／再接続表示に戻らない場合：Window A で `sudo systemctl restart toolmgmt.service` → ブラウザを Ctrl+Shift+R でハードリロード。`ERR_CONNECTION_REFUSED` が Pi5 復帰後も継続する場合は Pi5 のネットワーク名解決（`ping raspi-server.local`）を確認する。
