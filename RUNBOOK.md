...
1. **DocumentViewer 工程設定の保持方法**  
   ...
   - **TODO**: 設定編集 API・管理 UI の追加（工程リストの CRUD 含む）、設定ファイル初期化スクリプトの整備、DocumentViewer 側で新設定を読み込む処理。設定変更時は JSON 更新→即時通知で反映し、サービス再起動後も同じファイルを読み込む。Runbook/README の手順更新。設定ファイルが欠損・破損した場合は、UI 上で「工程未設定」を表示して再設定を促す。
