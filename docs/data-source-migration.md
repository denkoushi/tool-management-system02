...
| UI / 機能 | 主データソース | 取得方法 (tool-management-system02) |
| --- | --- | --- |
| 生産計画テーブル | RaspberryPiServer `/api/v1/production-plan` | `RaspiServerClient.get_json("/api/v1/production-plan")` |
| 標準工数テーブル | RaspberryPiServer `/api/v1/standard-times` | `RaspiServerClient.get_json("/api/v1/standard-times")` |
| 所在一覧テーブル | RaspberryPiServer `/api/v1/part-locations` | `RaspiServerClient.get_json("/api/v1/part-locations")` |
| 工程設定 (station) | RaspberryPiServer `/api/v1/station-config` | `load_station_config()` / `save_station_config()` が REST を利用 |

- `RASPI_SERVER_BASE` が未設定の場合は画面上に警告を表示し、テーブルは空表示となる（ローカル CSV/DB へのフォールバックは提供しない）。
- REST 失敗時は UI とログにエラーを表示し、RaspberryPiServer 側を確認して復旧する。
...
2. **クライアント側 (Window A)** _[完了]_
   - `build_production_view()` は REST API を前提とし、取得に失敗した場合はテーブルを空表示として警告を出す。
...
3. **ドキュメント / テスト** _[進行中]_
   - README / RUNBOOK / requirements に REST 連携方法と失敗時の警告表示を反映。
...
- RaspberryPiServer 障害時は UI で警告を表示し、復旧判断と切り戻しフローを RUNBOOK に明示する。
