...
- Pi5 停止中の再接続表示は未確認（OFFLINE 表示のまま）。更なる修正後に再テスト予定。
- 2025-11-01 (ウォッチドッグ適用後): `initRightPanel.js` の再接続ウォッチドッグにより、Pi5 停止中もチップが「再接続中…」表示を維持することを Pi4 実機で確認。Pi5 再起動後は自動で `LIVE` へ復帰。停止中はブラウザコンソールに `ERR_CONNECTION_REFUSED` が継続表示されるが想定内と整理。
- 2025-11-02 (状態マシン統合後): `socketStatusManager.js` の状態マシンでも同挙動を維持することを Pi4 実機で再確認。`SOCKET_STATUS_WATCHDOG=0` に変更した場合は `OFFLINE` へ降格することを確認し、既定値 1 に戻した。
