...
- **環境ファイルの配備**: `config/window-a-client.env.sample` を元に `sudo ./scripts/install_window_a_env.sh` を実行すると、Window A 用の設定ファイルを `/etc/toolmgmt/window-a-client.env` に展開できる。
