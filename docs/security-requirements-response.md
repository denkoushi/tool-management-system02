...
| --- | --- | --- | --- | --- | --- |
| 管理画面/API の認証 | 誰でもブラウザから USB 同期やシャットダウンを実行される危険。 | `API_AUTH_TOKEN` により管理用 API へトークン認証を追加。UI も初回操作時にパスコード入力。監査ログに成功/失敗を残す。 | 無関係な人が画面を触っても重要操作は実行できず、操作履歴も追跡できる。 | `curl -X POST /api/reset` をトークン無し/有りで叩き、401/200 を確認。ローテーション時は `python scripts/manage_api_token.py rotate --reveal` → Pi5 と Pi Zero の環境に同じ値を反映し、RUNBOOK 6 に従って再起動・記録する。必要に応じて `API_TOKEN_ENFORCE=0` で一時的に入力を省略できるが、セキュリティ低下に注意。 | RUNBOOK 3.4, 6 / docs/security-overview.md |
