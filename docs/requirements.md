...
- ⏳ **E2E テスト整備（Playwright）**
  ...
  - 2025-11-02: `tests/e2e/window-a-live.spec.ts` を実装し、環境変数が揃った場合に Pi4/Pi5 実機でスキャン連携と構内物流更新を検証できるようにした（未設定時は自動 skip）。同日、Pi4 実機で実行し PASS を確認 (`docs/test-notes/2025-11-02-window-a-live-playwright.md`)。
